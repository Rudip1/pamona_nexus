...
#
# Empty world, SDF pipeline — ORIGINAL model (stock Scout V2 wheels, no UV-C).
# Spawns from models/pomona_original/model.sdf (pre-baked). RSP still runs
# (pomona_state_publisher_original) so the TF tree is published.
#
# Run:  ros2 launch pomona_gazebo empty_world_pomona_original_sdf.launch.py
#
# NOTE: models/pomona_original/model.sdf is a placeholder until generated from
# urdf/pomona_original/xacro/pomona.xacro (xacro_to_urdf.sh + `gz sdf -p`).
...
def generate_launch_description():
    ...
    sdf_file   = os.path.join(pkg_pomona_gazebo, "models", "pomona_original", "model.sdf")
        ...
        actions=[
            IncludeLaunchDescription(
                ...
                launch_arguments={"x_pose": x_pose, "y_pose": y_pose,
                                  "theta": theta, "sdf_file": sdf_file}.items(),
            )
        ],
    )
